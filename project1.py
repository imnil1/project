...
def addingPlayerNameInFile(teamName, teamPlayers, file):
    file.write(f"\n\n\nTeam Name: {teamName}\n")  # Three newlines before team name
...
def Play(teamName, howManyOverTheMatchIs, howManyPlayersPlaying, opponentTeamName):
    ...
    while currentOver <= howManyOverTheMatchIs:
        ...
        elif choice == 41:  # Wide Ball / No Ball
            score += 1
...
def plotRunsGraph(team1Score, team1RunsPerOver, team1ExtrasPerOver, team1WicketsPerOver, team2Score, team2RunsPerOver, team2ExtrasPerOver, team2WicketsPerOver, team1Name, team2Name, filename='match_score_comparison.png'):
...
def choice1():
    try:
        with open('cricket.txt', 'r') as file:
            ...
            if allTheContent.strip():  # Check if there is content to display
                print(allTheContent)
                print("\n______________________________________\n")  # Added newline before the separator
            else:
...
def writeMatchResult(winner):
    try:
        with open("cricket.txt", "a") as file:
            file.write(f"\nMatch Result: {winner}\n")  # Single newline before match result
